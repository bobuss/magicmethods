...
## For forkers: ##
Edit `magicmethods.markdown`/`magicmethods.tex`, then run `make docs`. The build script requires the Python Markdown and pygments modules, so you'll have to run `pip install markdown` and `pip install pygments` if you don't already have it. Happy hacking!